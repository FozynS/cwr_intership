    ...
    <div id="page-sidebar">
      <div class="sidebar-wrap">
        <div class="sidebar-section sidebar-section-search">
          <div
            class="sidebar-section-header all-patients sidebar-patient-control"
          >
            <div
              class="inline-block"
            ...
            >
              All Patients
              <span v-if="searchedPatients" class="count"
                >({{ searchedPatients.patient_count }})</span
              >
            </div>
            <div>
              <el-button
          ...
          <div class="sidebar-section-container" id="search-block">
            <div class="form-group all-patients">
              <span
                class="patient-list-btn patient-list-btn-danger cancel-search"
                @click.prevent="cancelSearch()"
              ...
              <input
                type="text"
                id="patient-fast-search"
                class="form-control search"
                placeholder="Patient Search"
                v-model="searchQuery"
                @keyup="sendSearchRequest(selectedStatusesArray)"
                @keydown="clearTimer"
              />
              <div class="patient-status-filter" v-if="need_show_all_patients">
                <div class="inline-block">
                  <label
                    ...
                    v-for="status in patient_statuses"
                    :style="{ color: getStatusColor(status) }"
                  >
                    <input
                      type="checkbox"
                      v-model="selected_statuses[status.status]"
            ...
            <div
              class="sidebar-section-container-scrollable"
              :class="{ active: need_show_all_patients }"
            >
              <div class="container-patients-list">
                <div
                  v-show="
                    need_show_all_patients &&
                    patient_statuses_blocks_visible[key] !== false &&
                    selected_statuses[key] !== false
                  "
                ...
                >
                  <div class="sidebar-section-header">
                    {{ key }} Patients ({{
                      searchedPatients.statuses_count[key]
                    }})
                  </div>

                  <div class="sidebar-section-container patients-container">
                    <table
                      class="table sidebar-patients-list sidebar-patients-list-search"
                      :id="`${'status-' + key}`"
                    >
                      <thead>
                        <tr>
                      ...
                      <tbody class="list">
                        <tr
                          ...
                          :class="{
                            active: $route.params.id == searchedPatient.id,
                          }"
                        ...
                        >
                          ...
                          <router-link
                            :title="searchedPatient.full_name"
                            tag="td"
                          ...
                          >
                            <a
                              :style="{
                                color: '#' + searchedPatient.status_color,
                              }"
                              class="patient-name"
                            >
                          ...
                          <td>
                            {{
                              getFormattedDate(searchedPatient.date_of_birth)
                            }}
                          </td>
                        </tr>
                      </tbody>
                ...
                <div
                  v-if="
                    paginationSearched &&
                    paginationSearched.next_page_url &&
                    need_show_all_patients
                  "
                  v-observe-visibility="
                    !loading_search_block && !searchScrolled
                      ? loadMoreSearched
                      : false
                  "
                  class="load-more absolute"
                ></div>

                <div
                  class="text-center sidebar-loader-wrapper-list"
                  v-if="need_show_all_patients"
                >
                  <pageloader
                    add-classes="sidebar-loader"
                    v-if="loading_search_block && need_show_all_patients"
                  ></pageloader>
                </div>
              </div>
            </div>
        ...
        <div
          ...
          v-show="!need_show_all_patients"
        >
          <div class="sidebar-section-header">
            Messages
            <span class="count" v-if="paginationMessages"
              >({{ paginationMessages.total }})</span
            >
            <span
              class="sidebar-loader-wrapper"
              v-if="loading_messages_block && !paginationMessages"
            >
          ...
          <div
            class="sidebar-section-container"
            id="messages-list"
            v-if="messages.length"
          >
            <div
              class="patients-container"
              :class="{ 'max-15': isMoreThan15(messages) }"
            >
              <table class="table sidebar-patients-list">
                ...
                <tbody>
                  <template v-for="(message, index) in messages">
                    <tr
                      v-if="
                        paginationMessages.next_page_url &&
                    ...
                    >
                      <td colspan="3">
                        <div
                          v-observe-visibility="
                            !messageScrolled ? loadMoreMessages : false
                          "
                        ></div>
                      </td>
                    </tr>
                    <tr
                      v-if="message.model === 'PatientLeadComment'"
                      class="active"
                    >
                      <td
                        class="patient-link text-preview-container"
                      ...
                      >
                        <a
                          target="_blank"
                          :href="`/new-patients-dashboard?selectedCard=${message.comment.patient_lead.inquiry.id}&comment=${message.comment.id}`"
                          style="color: black"
                        >
                          <span
                            >{{ fullName(message.comment.patient_lead) }} (Not
                            created)</span
                          >
                        </a>
                      </td>
                      ...
                      >
                        <a
                          ...
                          style="color: black"
                        >
                          <span v-if="message.comment.provider">{{
                            message.comment.provider.provider_name
                          }}</span>
                          <span v-else-if="message.comment.provider_name">{{
                    ...
                    <tr
                      ...
                      :class="{
                        active:
                          $route.params.id == message.comment.patient_id &&
                          $route.query.scrollto ==
                            message.model + message.comment_id,
                      }"
                    >
                      <td
                        class="patient-link text-preview-container"
                      ...
                      >
                        <a
                          :href="
                            '/chart/' +
                            message.comment.patient_id +
                            '?scrollto=' +
                            message.model +
                            message.comment_id
                          "
                          :style="{
                            color: '#' + statusColor(message.comment.patient),
                          }"
        ...
        <div class="sidebar-section" v-show="!need_show_all_patients">
          <div class="sidebar-section-header">
            ...
            <span class="count" v-if="todayPatients.length"
              >({{ todayPatients.length }})</span
            >
            <span
              class="sidebar-loader-wrapper"
          ...
          <div
            class="sidebar-section-container p-0"
          ...
          >
            <div class="patients-container">
              <table class="table sidebar-patients-list">
                <tbody>
                  <tr
                    v-for="(todayPatient, index) in todayPatients"
                    :key="`todayPatient.id-${index}`"
                    :class="{ active: $route.params.id == todayPatient.id }"
                  >
                    <router-link
                      tag="td"
                    ...
                    >
                      <a
                        :style="{
                          color: '#' + todayPatient.status_color,
                          display: 'inline-block',
                        }"
                        :class="{
                          'appt-complete': isAppointmentComplete(todayPatient),
                        }"
                      >
                    ...
                    <td
                      class="appt-buttons-container"
                      v-if="!isAppointmentComplete(todayPatient)"
                    ...
                    >
                      ...
                      <span
                        class="patient-list-btn patient-list-btn-warning"
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("getCurrentProvider").then(() => {
      if (
        this.$store.state.currentProvider &&
        this.$store.state.currentProvider.id
      ) {
        window.Echo.private(
          `providers.${this.$store.state.currentProvider.id}.appointments`
        ).listen(".appointments.today.updated", () => {
          this.$store.dispatch("getProviderTodayPatients");
        });
      } else {
        //admin channel
        window.Echo.private("appointments").listen(
          ".appointments.today.updated",
          () => {
            this.$store.dispatch("getProviderTodayPatients");
          }
        );
    ...
    this.$store.dispatch("getPatientStatuses").then(() => {
      for (let i in this.patient_statuses) {
        this.$set(
          ...
          this.patient_statuses[i].status,
          true
        );
      }
    });
    ...
    this.$store.dispatch("getProviderMessages").then(() => {
      this.loading_messages_block = false;
    });
    this.$store.dispatch("getProviderTodayPatients").then(() => {
  ...
  methods: {
    ...
    sendSearchRequest(statuses = null) {
      ...
      this.typingTimer = setTimeout(() => {
        ...
        self.$store
          .dispatch("getSearchedPatients", payload)
          .then((response) => {
    ...
    loadMoreMessages(isVisible, entry) {
      if (isVisible) {
        ...
        this.$store
          .dispatch(
            "getProviderMessages",
            this.paginationMessages.current_page + 1
          )
          .then(() => {
    ...
    isAppointmentComplete(patient) {
      ...
      if (this.statusesForTodayPatients.complete.indexOf(status_id) !== -1) {
        ...
        this.statusesForTodayPatients.cancel.indexOf(status_id) !== -1
      ) {
        return 2;
      } else if (
        this.statusesForTodayPatients.reschedule.indexOf(status_id) !== -1
      ) {
        return 3;
      }
      return 0;
